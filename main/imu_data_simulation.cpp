...
int main(int argc, char** argv) {
  ...
  const std::string bag_path = output_path + "imu_noise_data_sim.txt";
  ...
  for (double t = params.t_start; t < params.t_end;) {
    if ((int)t % params.imu_frequency == 0) {
      int i =
          (int)((t - params.t_start) / (params.t_end - params.t_start) * 100);
    ...
    ofs << std::fixed << std::setprecision(6) << begin + t << " "
        << std::setprecision(7) << data_noise.imu_acc(0) << " "
        << data_noise.imu_acc(1) << " " << data_noise.imu_acc(2) << " "
        << data_noise.imu_gyro(0) << " " << data_noise.imu_gyro(1) << " "
