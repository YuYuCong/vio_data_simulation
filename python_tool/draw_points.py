...
with open(filepath + '/cam_pose.txt', 'r') as f:  # imu_circle   imu_spline
    ...
    for line in data:
        ...
        quaterntions.append(
            [numbers_float[qw_index], numbers_float[qw_index + 1],
             numbers_float[qw_index + 2],
             numbers_float[qw_index + 3]])  # qw,qx,qy,qz
        position.append(
            [numbers_float[qw_index + 4], numbers_float[qw_index + 5],
             numbers_float[qw_index + 6]])
...
for i in range(0, 400, 5):
    ...
    with open(s, 'r') as f:
        ...
        for line in data:
            ...
            ax.plot([numbers_float[0], p[0]], [numbers_float[1], p[1]],
                    zs=[numbers_float[2], p[2]])
    ...
    with open(s, 'r') as f:
        ...
        for line in data:
            odom = line.split()  # 将单个数据分隔开存好
            numbers_float = list(map(float, odom))  # 转化为浮点数
            ax.plot([numbers_float[0], numbers_float[3]],
                    [numbers_float[1], numbers_float[4]], 'b',
                    zs=[numbers_float[2], numbers_float[5]])
